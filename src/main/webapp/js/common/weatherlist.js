	function getFileSystem(){
		var file = '/tomcat/experiment/weathercloud/uploaddata';
		      ...
		      var tr1 = "<dd class='list-view-item'>"; 
				...
				tr1 += "<span class='fileicon'></span>";
					tr1 += "<div class='file-name' style='width:62%'>";
						tr1 += "<div class='text'>";
							tr1 += "<a href='detailweather?filename="+data[i].pathSuffix+"' title='"+data[i].pathSuffix+"'>"+data[i].pathSuffix+"</a>";
						tr1 += "</div>";
						tr1 += "<div class='operate'>";
						tr1 += "<a class='icon icon-download-blue' href='downloadweather?filename="+data[i].pathSuffix+"' title='下载'></a>";
	...
	function downloadAllAction(command){
		...
		form.action="downloadmutilweather";
		form.method= "get"; 
		var value=0;
		$("input[name='filename']").each(function () {
			if(this.checked){
