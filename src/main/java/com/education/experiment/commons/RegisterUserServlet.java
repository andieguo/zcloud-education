...
public class RegisterUserServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ub.setUserName(new String(request.getParameter("userName").getBytes("ISO-8859-1"), Charset.defaultCharset()));
		ub.setPhoneNumber(request.getParameter("phone"));
		ub.setRemark(new String(request.getParameter("remark").getBytes("ISO-8859-1"), Charset.defaultCharset()));
		...
		try {
			...
			// // 建表
			// st.close();
			PreparedStatement ps = conn.prepareStatement("insert into USER_INFO(ID,PASSWD,NAME,CLOUDSIZE,PHONENUM,REMARK) values (?,?,?,?,?,?)");// 插入数据
